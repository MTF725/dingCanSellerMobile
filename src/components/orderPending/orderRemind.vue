<template>
	<div class="index" style="padding-bottom: 45px;position: relative;">		
		<van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="height:calc(100% - 62px;);">
			<van-list v-model="loading" :finished="finished" finished-text="没有更多了" offset=1  @load="getOrderRemind">		
				<!-- 订单列表 -->
	...
	export default{
		...
		methods:{
			...
			async getOrderRemind(){		
				...
				this.pageNum++;
				await this.$get('getOrderRemind',{sellerId:this.$global.sellerId,pageNum:this.pageNum,pageSize:20}).then(res=>{					
					if(res.status=="200"){	
