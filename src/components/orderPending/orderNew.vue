<template>
	<div class="index" style="padding-bottom: 45px;position: relative;">		
		<van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="height:calc(100% - 62px;);">
			<van-list v-model="loading" :finished="finished" finished-text="没有更多了" offset=1  @load="getOrderNew">			
				<!-- 订单列表 -->
				<div class="order-detail" v-for="(item,index) in allMsgData" :key="index">
					...
					<div class="button-group"> 
						<span class="operate-btn" @click="cancelOrderBtn(item.msg1.orderNo)" >取消</span>
						<span class="operate-btn" @click="confirmOrderBtn(item.msg1.orderNo)" style="background-color:#F4BE42;border-color: #F4BE42;">确定接单</span>
	...
	export default{
		...
		methods:{								
			...
			async getOrderNew(){	
				...
				this.pageNum++;
				await this.$get('getOrderNew',{sellerId:this.$global.sellerId,pageNum:this.pageNum,pageSize:20}).then(res=>{					
					if(res.status=="200"){	
			...
			confirmOrderBtn(e){	
				...
				this.$post("orderAccept",{orderNo :e}).then(res=>{
					if(res.status==200){
						this.$toast("单号"+e+"已接单");
						for(var i =0;i<this.allMsgData.length;i++){
							if(this.allMsgData[i].msg1.orderNo ==e){
		...
		mounted() {
			...
			if ('WebSocket' in window) {
				this.websocket = new WebSocket('ws://'+this.$global.wsPort+'/imserver/'+this.$global.sellerId);				
				this.initWebSocket()
			} else {
				alert('当前浏览器 不支持')
			 }
