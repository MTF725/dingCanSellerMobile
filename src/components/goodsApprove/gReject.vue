<template>
	...
	<div class="box" style="background: #F9F9F9;">
		<div class="goods-deal-title" style="background: white;">			
			<p><span @click="patchDelGoods">批量删除</span></p>							
		</div>
							...
							<div class="goods-msg-right">
								...
								<div class="list-goods-btns">
									<div class="reject-btns">
										<van-button plain color="#D2D2D2" type="primary" @click="gEdit(index)" size="small">整改</van-button>
										<van-button plain color="#D2D2D2" type="primary" @click="sellOnEdit(index)" size="small">详情</van-button>
										<van-button plain color="#F26666" type="primary" @click="delGoods(index)" size="small">删除</van-button>
									</div>				
								</div>					
		...
		methods:{		
			...
			getOperateList(){
				...
				let operateListData = {
					...
					gAuditState : 2 , //审核状态0审核中1已审核2已驳回
					keyword : "",
			...
			//整改
			gEdit(e){
				this.$router.push({path:"/goodBuild",query:{goodsData:this.goodsOperateArr[e],fromBtn:"商品审批整改"}})
			},
			...
			//删除商品
			delGoods(e){
				let ids =[this.goodsOperateArr[e].gId];	
				this.$dialog.confirm({
					title: "确定删除？",
				}).then(() => {
					this.$toast.loading({
				    ...
				    this.$delArr("goodsDel",{gIds:ids}).then(res=>{
				    	if(res.status==200){
				    		this.$toast("删除成功");
				    		this.clearGetList();//查找已驳回商品数据
				    	}
				    }).catch(err=>{
				    	console.log(err)
				    })	
...
<style type="text/css" scoped>
	...
	.goodsApprovePopup .list-goods-btns button:not(:last-of-type) span{color:#333333;}
	...
	.list-goods-btns .van-button--small {min-width: 52px;height: 28px;line-height:27px;}
