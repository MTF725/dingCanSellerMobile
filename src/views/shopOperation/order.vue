<template>
	<!-- 账单弹窗 -->
	<div class="orderAccountPopup" style="height:100%">
		<!-- 顶部导航 -->
		<van-nav-bar title="账单" left-text="门店运营" fixed placeholder right-text="" left-arrow @click-left="navTopBack"  />	
		<!-- popup内容区 -->
		<div class="popup-main" style="height: calc(100% - 40px);">
			<!-- 订单上部时间选择 -->
		    <div class="order-time-switch-empty">
				<div class="order-time-switch">
					<div class="ots-top"><span>{{currentDate}}</span><span class="iconfont icon-jiantou9" @click="yearMonthSelect=true"></span></div>
					<div class="ots-bottom">
						<p><span>收入</span><span><i>￥</i><i>2000</i></span></p>
						<p @click="toOrderMonthPopup"><span>月账单</span><span class="iconfont icon-zuojiantou"></span></p>
					</div>							
				</div>
			</div>
			<!-- 订单下部列表 -->
			<van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="height:calc(100% - 62px;);">	
				<ul class="order-list" style="height: 100%">
					<van-list v-model="loading" :finished="finished" finished-text="没有更多了" offset=1 @load="getFinishOrders">
						<li v-for="(item,index) in ordersFinish" :key="index" class="van-clearfix">
							<div class="order-list-num">{{item.ordersFinishNo}}</div>
							<div class="order-list-right">
								<div class="olr-no">
									<div><span>订单号：</span><span>{{item.orderNo}}</span></div>								
								</div>
								<div class="olr-time">
									<p>{{item.createTime}}</p>
									<p>+{{ordersSellerGetArr[index]}}</p>
								</div>						
							</div>
						</li>
					</van-list>		
...
<script>
	export default{
		...
		methods:{
			...
			//跳转到月账单详情			
			toOrderMonthPopup(){
				this.$router.push({name:"orderMonthDes",params:{date:this.currentDate}})
			},
							
			//年月确定
			yearMonthConfirm(e){					
				this.currentDate = this.dealTime(e);
				this.currentDatePicker = this.currentDates;
			...
			getFinishOrders(){
				if (this.refreshing) {
				  this.ordersFinish = [];
				  this.ordersSellerGetArr = [];
				  this.refreshing = false;
				  this.orderTotalNum=0;//总交易笔数
				  this.pageNum=0;//页码
				  this.ordersFinishNo=0;
				}
				this.loading = true;						
				if (this.pageNum>1&&this.ordersFinish.length >= this.orderTotalNum) {
					this.finished = true;
					this.loading = false;
					return
				}
				...
				this.$get("getFinishOrders",data).then(res=>{
					if(res.status ==200){						
						...
						this.loading = false;		
						console.log(this.ordersFinish.length)
					}
				}).catch(err=>{
					console.log(err)
				})
...
<style scoped="scoped">
	...
	.order-time-switch{padding:10px 15px;position: fixed;top: 45px;left: 0;right: 0;background: #EEEEEE;z-index: 10;}
